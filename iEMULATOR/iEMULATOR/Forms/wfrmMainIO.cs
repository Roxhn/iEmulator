...
{
    public partial class wfrmMainIO : Form
    {
        public wfrmMainIO()
        {
            InitializeComponent();
        }
        private void wfrmMainIO_Load(object sender, EventArgs e)
        {
            fill();
        }
        ...
        private void fill()
        {
            try
            {
                ...
                this.vIEW_PRCIOTableAdapter.Fill(this.dtsEmulator.VIEW_PRCIO);
            }
            catch (Exception ex)
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (validate())
                {
                    try
                    {
                        this.pRC_IOTableAdapter.insertIO((int?)this.comboBox1.SelectedValue, this.textBox1.Text, Convert.ToInt32(this.textBox3.Text), Convert.ToInt32(this.textBox4.Text));
                        fill();
                    }
                    catch (Exception)
                    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.dataGridView1.RowCount > 0)
                {
                    ...
                    {
                        this.pRC_IOTableAdapter.DeleteIO(Convert.ToInt32(this.dataGridView1.CurrentRow.Cells["ID_IO"].Value));
                        fill();
                    }
                }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.RowCount > 0)
                {
                    ...
                    this.comboBox1.SelectedValue = this.dataGridView1.CurrentRow.Cells["ID_IO_MAYOR"].Value;
                    if (this.dataGridView1.CurrentRow.Cells["ID_IO_MAYOR"].Value.ToString().Trim().Equals(""))
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (validate())
            {
                try
                {
                    this.pRC_IOTableAdapter.updateIO((int?)this.comboBox1.SelectedValue, this.textBox1.Text, Convert.ToInt32(this.textBox3.Text), Convert.ToInt32(this.textBox4.Text), (int)this.dataGridView1.CurrentRow.Cells["ID_IO"].Value);
                    allowdenied(true);
                    fill();
                    clear();
                }
